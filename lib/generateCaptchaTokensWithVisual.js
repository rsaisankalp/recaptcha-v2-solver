...
async function analyzeWithGemini(screenshotPath, prompt, gridType, geminiConfig) {
    try {
        ...
        const model = genAI.getGenerativeModel({
            model: geminiConfig.model || 'gemini-flash-lite-latest',
            generationConfig: {
                temperature: 0.1,
                topP: 0.95,
                topK: 40,
...
async function generateCaptchaTokensWithVisual({
    ...
    if (!geminiConfig.apiKey) throw new Error('Gemini API key is required');
    ...
    let browser;
    try {
        // Step 1: Launch a single browser instance.
        browser = await launchBrowser(
            `${browserConfig.userDataDir}/chrome-user-data-1`,
            null, // Assuming no proxy
            browserConfig
        );
        ...
        // Step 3: Go to the target URL.
        await page.goto(captchaUrl, {
            waitUntil: 'domcontentloaded',
            timeout: 120000
        });

        // Step 4: Solve the captcha on that specific page.
        const token = await solveCaptchaChallenge(page, geminiConfig);

        if (token) {
            eventEmitter.emit('tokenGenerated', { token });

            // Step 5: If the callback exists, execute it with the live page and token.
            // The browser will be closed inside the callback itself.
            if (onTokenGenerated) {
                await onTokenGenerated(token, page);
            } else {
                // If no callback is provided, we are done. Close the browser.
                await browser.close();
            }
        } else {
            logger.error('Failed to generate a token.');
            eventEmitter.emit('tokenError', { error: 'Failed to solve captcha' });
            if (!onTokenGenerated) { 
                await browser.close();
            }
        }
    } catch (error) {
        logger.error('An error occurred in generateCaptchaTokensWithVisual:', error);
        eventEmitter.emit('tokenError', { error: error.message });
        if (browser && browser.isConnected() && !onTokenGenerated) { 
             await browser.close();
        }
    }
}
